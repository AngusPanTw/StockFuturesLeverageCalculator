    ...
    public partial class BatchImportWindow : Window
    {
            ...
            List<BatchImportItem> items, List<string> errors, string blockLabel)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (parts.Length < 3)
                {
                    errors.Add($"[{blockLabel}] 第 {lineNum} 行格式錯誤: \"{line}\"（需要：股票代號 口數 進場價格）");
                    continue;
                }
                ...
                if (!decimal.TryParse(parts[2], out decimal costPrice) || costPrice < 0)
                {
                    errors.Add($"[{blockLabel}] 第 {lineNum} 行進場價格無效: \"{parts[2]}\"");
                    continue;
                }
